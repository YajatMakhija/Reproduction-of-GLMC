"""Linear mode connectivity of two ViTs on CINIC-10 (GLMC)."""
...
def _looks_like_cinic(p):
    return (p / "train").is_dir() and (p / "test").is_dir()

def resolve_cinic10_root(root):
    p = Path(root)
    if _looks_like_cinic(p):
        return str(p)
    for child in (p.iterdir() if p.is_dir() else []):
        if child.is_dir() and _looks_like_cinic(child):
            return str(child)
    for child in p.rglob("*"):
        if child.is_dir() and _looks_like_cinic(child):
            return str(child)
    raise FileNotFoundError(f"CINIC-10 train/test not found under {root}")

def _decode_split(split_dir, cache_path):
    """One-time PNG decode of a split into a uint8 tensor, cached on disk."""
    ...
    print(f"[data] decoded {Path(split_dir).name}: {tuple(images.shape)} -> cached {cache_path}")
...
class GPULoader:
    ...
    def __init__(self, images, labels, batch_size, shuffle, augment=None,
                 drop_last=False):
...
def get_cinic10_loaders(root, device, batch_size=128, test_batch=2048,
                        combine_train_valid=False, cache_dir="cinic10_cache"):
    cinic_root = Path(resolve_cinic10_root(root))
    cache = Path(cache_dir)
    cache.mkdir(exist_ok=True)
    tr_imgs, tr_lbls = _decode_split(cinic_root / "train", cache / "train.pt")

    st_imgs, st_lbls = tr_imgs.to(device), tr_lbls.to(device)
    if combine_train_valid and (cinic_root / "valid").is_dir():
        v_imgs, v_lbls = _decode_split(cinic_root / "valid", cache / "valid.pt")
        tr_imgs = torch.cat([tr_imgs, v_imgs])
        tr_lbls = torch.cat([tr_lbls, v_lbls])
    te_imgs, te_lbls = _decode_split(cinic_root / "test", cache / "test.pt")
    tr_imgs, tr_lbls = tr_imgs.to(device), tr_lbls.to(device)
    te_imgs, te_lbls = te_imgs.to(device), te_lbls.to(device)
    ...
    trainloader = GPULoader(tr_imgs, tr_lbls, batch_size, shuffle=True,
                            augment=augment, drop_last=True)
    statloader = GPULoader(st_imgs, st_lbls, batch_size, shuffle=True)
    testloader = GPULoader(te_imgs, te_lbls, test_batch, shuffle=False)
    print(f"[data] CINIC-10 root={cinic_root}  train={tr_imgs.shape[0]:,}  "
          f"test={te_imgs.shape[0]:,}  (all resident on {device})")
    return trainloader, statloader, testloader, 10
...
raw_path = kagglehub.dataset_download("mengcius/cinic10")
print("Path to dataset files:", raw_path)

trainloader, statloader, testloader, num_classes = get_cinic10_loaders(
    raw_path,
    device,
...
r0 = reparameterize(model0, num_classes).to(device).eval()
r1 = reparameterize(model1, num_classes).to(device).eval()
heads = cfg["heads"]
...
def _lambda_plot(data, ylabel, fname):
    ...
    ax.set_title("CINIC-10 · Linear Mode Connectivity of two ViTs")
...
fig_loss = _lambda_plot(curves, "Test loss", "cinic10_loss_vs_lambda.png")
fig_acc = _lambda_plot(acc_curves, "Test accuracy (%)", "cinic10_acc_vs_lambda.png")
...
fig_tr.suptitle("CINIC-10 · Base ViT training (seeds 0 & 1)")
fig_tr.tight_layout()
fig_tr.savefig(PLOTS_DIR / "cinic10_training_curves.png", dpi=200,
...
(OUT_DIR / "cinic10_interp_curves.json").write_text(json.dumps(serial, indent=2))
print(f"[save] plots -> {PLOTS_DIR}  curves+barriers -> {OUT_DIR / 'cinic10_interp_curves.json'}")
print("[barriers]")
...
def _zip_results():
    zip_path = Path("cinic10_results.zip")
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        for f in sorted(Path("outputs_cinic10").rglob("*")):
            if f.is_file():
